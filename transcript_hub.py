...
with tab4:
    ...
    if all_themes:
        ...
        co_matrix = {theme: [0] * len(all_themes) for theme in all_themes}

        for themes in theme_sets:
            for i, ti in enumerate(all_themes):
                for j, tj in enumerate(all_themes):
                    if ti in themes and tj in themes:
                        co_matrix[ti][j] += 1

        # Convert to DataFrame for heatmap
        import pandas as pd
        co_df = pd.DataFrame(co_matrix, index=all_themes, columns=all_themes)
