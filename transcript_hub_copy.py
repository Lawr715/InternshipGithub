...
with tab1:
    ...
    if not st.session_state.get("clear_uploaded_transcript", False):
        ...
        if uploaded_file:
            # Add a field for transcript name/title
            ...
            if st.button("Upload Transcript"):
                st.session_state.transcripts.insert(0, {
                    "name": transcript_name,  # Add the name field
                    "text": text,
                    "location": location,
                    "type": t_type,
                    "date": date_str,
    ...
    if st.session_state.transcripts:
        ...
        for idx, transcript in enumerate(st.session_state.transcripts):
            ...
            st.download_button(
                f"Generate DOCX for Transcript {idx+1} ({transcript.get('name', '')} {transcript['date']} • {transcript['type']} • {transcript['location']})",
                docx_buffer,
                file_name=f"{transcript.get('name','').replace(' ','_')+'_' if transcript.get('name') else ''}{transcript['date']}_{transcript['type']}_{transcript['location'].replace(' ', '_')}.docx",
...
with tab4:
    ...
    st.subheader("📈 Theme Cooccurrence Heatmap")
